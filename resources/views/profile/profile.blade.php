    ...
    document.getElementById('pictureInput').addEventListener('change', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Keep the shared sidebar (visible on every dashboard) in
                // sync immediately, rather than waiting for a full reload.
                if (typeof window.updateSidebarAvatar === 'function') {
                    window.updateSidebarAvatar(data.profile_picture_url, null);
                }
            } else {

...
<script>
    ...
    async function loadBrowsePosts() {
        ...
        // Use the exclusion-aware endpoint (PostController@index), which
        // filters out posts excluded from this viewer, instead of the raw
        // topic->posts relation (which does not apply that filter).
        const data = await api(`/topics/${activeBrowseTopicId}/posts`);
        if (!data) {
            container.innerHTML = '<div class="muted">This topic could not be loaded.</div>';
            return;
        }

        const myId = window.CURRENT_USER ? window.CURRENT_USER.user_id : null;
        // API returns newest-first (latest('posted_at')); reverse to
        // chronological order so new messages render at the bottom, like
        // a normal chat thread.
        const posts = ((data && (data.data || data)) || []).slice().reverse();
